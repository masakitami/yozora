...
class Cell {
  ...
  constructor(DOM_el, { position, previousCellPosition } = {}) {
    this.DOM.el = DOM_el;
    // console.log("=======確認======", this.DOM.el);
    // if (!this.DOM.el.classList.contains("char")) {
...
export class TypeShuffle {
  ...
  fx6() {
    ...
    const loop = (line, cell, iteration = 0) => {
      ...
      if (iteration === MAX_CELL_ITERATIONS - 1) {
        ...
        cell.color = ["#14532D", "#88FFA2", "#DCFCE7"][Math.floor(Math.random() * 3)];
        cell.DOM.el.style.color = cell.color;
      }
    ...
    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(() => {
          ...
          loop(line, cell), (line.position + 1) * 80;
        });
      }
    }
  }
  ...
  fx7() {
    // // max iterations for each cell to change the current value
    this.DOM.el.classList.remove("invisible");
    ...
    const loop = (line, cell, iteration = 0) => {
      ...
      ++iteration;
      if (iteration < MAX_CELL_ITERATIONS) {
        setTimeout(() => loop(line, cell, iteration), randomNumber(30, 40));
      }
    };
    ...
    for (const line of this.lines) {
      for (const cell of line.cells) {
        setTimeout(() => {
          cell.DOM.el.classList.remove("invisible");
          loop(line, cell), (line.position + 1) * 8000;
        }, 2000);
      }
    }
  }
